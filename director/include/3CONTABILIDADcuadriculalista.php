            <div id="alumnos-cuad" class="alumnos-master"> 
                <!--<div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>ALARCON ZURITA NIDIA</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>ALARCON ZURITA NIDIA</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>BAIDAL SUAREZ JOSUE SANTIAGO</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>BECERRA SANCHEZ DAVID SEBASTIAN</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>CABALLERO CHILA MARIO ISRAEL</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>CALDERON CARRERA DAVID ESTEBAN</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>DAQUILEMA NAVARRO JEREMIE ELIAS</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>DUEÑAS CHOEZ JAIME IVAN</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>GARCIA RAMONES CARLOS JAVIER</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>HABLICH PAREDES ANDREA MELISSA</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>LOGROÑA ORTIZ BRUNO MAURICIO</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>MENDOZA AVILEZ LISSETTE PRISCILA</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>MOREIRA PARRALES BRYAN NORBERTO</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>ORELLANA ROJAS MAYERLI ADALIZ</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>PEREZ NUÑEZ GERSSON ALEXANDER</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>SOLORZANO MENDIETA GRACE STEFANIE</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>URRUTIA MOLINA DIANA CAROLINA</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>VASQUEZ RODRIGUEZ YELENA MARIA</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>VELASCO CALERO WILSON GARLI</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>VELEZ GARCIA ANTHONY ISMAEL</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>
                <div class="alumnos-cuadricula">
                    <div class="alumno">
                        <img src="../img/icono_persona.png" alt="" width="25">
                        <h4>ZAMORA GUSMAN DAYANA GISSELLE</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                    </div>
                    <div class="representante">
                        <img src="../img/representante.png" alt="" width="25">
                        <h4>Nombres y Apellidos</h4>
                        <a href="">
                            <button>Perfil</button>
                        </a>
                        <a href="">
                            <button>Representante</button>
                        </a>
                    </div>
                </div>-->

            </div>

<script type="text/javascript">


     var xhttp = new XMLHttpRequest();
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          getCourse(this);
          
        }
      };
      xhttp.open("GET", "xml/course3CONTABILIDAD.xml", true);
      xhttp.send();

      function getCourse(xml){
        var i,j,k;
          var xmlDoc = xml.responseXML;
         //var table="<p><strong>Profesores</strong></p>";
          var x = xmlDoc.getElementsByTagName("student");
          for (i = 0; i <x.length; i++) { 
            var newdiv = document.createElement('div');
            newdiv.setAttribute('class',"alumnos-cuadricula")


            var divAl = document.createElement('div');
            divAl.setAttribute('class',"alumno");


            var imgAl=document.createElement('img');
            imgAl.setAttribute('alt'," ");
            imgAl.setAttribute('src',"../img/icono_persona.png");
            imgAl.setAttribute('width',"25");

            divAl.appendChild(imgAl);

            var h4Al = document.createElement('h4');
            h4Al.innerHTML=x[i].getElementsByTagName("name")[0].childNodes[0].nodeValue + " " + x[i].getElementsByTagName("lastName")[0].childNodes[0].nodeValue;

            divAl.appendChild(h4Al);

            var aAl=document.createElement('a');
            aAl.setAttribute('href',"");

            var btnAl = document.createElement('button');
            btnAl.innerHTML="Perfil";

            aAl.appendChild(btnAl);

            divAl.appendChild(aAl);



            var divRep=document.createElement('div');
            divRep.setAttribute('class',"representante");

            var imgRep=document.createElement('img');
            imgRep.setAttribute('alt'," ");
            imgRep.setAttribute('src',"img/REPRESENTANTE.png");
            imgRep.setAttribute('width',"25");

            divRep.appendChild(imgRep);

            var h4Rep = document.createElement('h4');
            h4Rep.innerHTML="Nombres y Apellidos";

            divRep.appendChild(h4Rep);

            var aRep=document.createElement('a');
            aRep.setAttribute('href',"");

            var btnRep = document.createElement('button');
            btnRep.innerHTML="Perfil";

            aRep.appendChild(btnRep);

            var aRep2=document.createElement('a');
            aRep2.setAttribute('href',"");

            var btnRep2 = document.createElement('button');
            btnRep2.innerHTML="Representante";

            aRep2.appendChild(btnRep2);

            divRep.appendChild(aRep);
            divRep.appendChild(aRep2);

            newdiv.appendChild(divAl);
            newdiv.appendChild(divRep);

            document.getElementById("alumnos-cuad").appendChild(newdiv);


          }



      }
</script>